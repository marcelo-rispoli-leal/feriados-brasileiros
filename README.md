...
ok - A terça-feira de carnaval ocorre 47 dias antes do domingo de Páscoa
ok - corpus christi ocorre 60 dias após o domingo de Páscoa
ok - A sexta-feira santa ocorre 2 dias antes do domingo de Páscoa

ok - A Páscoa é celebrada no primeiro domingo após a primeira lua cheia que ocorre
...
ok - Os feriados nacionais com data fixa são:
...
ok - Para resolver esse problema você deve desenvolver uma API que permita consultar
e cadastrar feriados estaduais e municipais.

ok - O endpoint para consultar feriados deve seguir o seguinte formato:

```
/feriados/CODIGO-IBGE/ANO-MES-DIA/
```

ok - Onde CODIGO-IBGE pode ser um número de dois dígitos, para representar um
feriado estadual ou um número com 7 dígitos para representar um feriado
municipal.

ok - O MES deve ser um mês válido, ou seja, entre 1 e 12. Mesmo cuidado deve ser
tomado com o dia. Espere uma ano com 4 números, mês com 2 números e dia também
com 2 números, ou seja, "AAAA-MM-DD".

ok - A sua API deve ser populada com os feriados nacionais. Todos os feriados
estaduais e municipais consultados nos testes serão criados através da API.

ok - Esse endpoint deve responder os seguintes verbos: GET, PUT e DELETE.

ok - Para simplificar o problema, assuma que pode haver no máximo 1 feriado por dia
em cada município, e que serão feitas consultas apenas em dias de semana, ou
seja, de segunda a sexta.

ok - O comportamento esperado do GET é que ele retorne status 200 e o nome do
...
ok - Se não houver um feriado no dia para o estado ou município consultado a API deve retornar 404.

ok - O cadastro de um feriado estadual ou municipal segue estrutura semelhante
...
ok - A API deve retornar o status 200 para indicar que a requisição foi bem
sucedida.

ok - Opcionalmente ela pode retornar 201 se esse feriado ainda não estava
cadastrado na base.

ok - Se já existir um feriado cadastrado neste dia para o estado
ou município especificado, o nome do feriado deve ser atualizado.

ok - Deve haver também a opção de apagar um feriado.
