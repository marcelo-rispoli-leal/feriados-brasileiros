...
async function checkCod(myCod) {
  ...
  if (myCod.length === 2) {
    ...
    ret = file.estados.find((a) => a.cuf === myCod);
...
async function getCarnival(myCod) {
  ...
  const holiday = holidays.feriados.find((a) => a.cod === myCod);
...
router.put('/:cod/:date', async (req, res, next) => {
  try {
    ...
    if (myDate === 'carnaval' || myDate === 'corpus-christi') {
      ...
      holiday = holidays.feriados.find((a) => a.cod === myCod);
      ...
      if (!holiday) {
        ...
        await writeFile(fileName, JSON.stringify(holidays, null, 2));
...
router.delete('/:cod/:date', async (req, res, next) => {
  try {
    ...
    if (myDate === 'carnaval' || myDate === 'corpus-christi') {
      ...
      holidays = JSON.parse(await readFile(fileName));
      holiday = await holidays.feriados.find((a) => a.cod === myCod);
    } else {
      if (myCod.length === 2) {
        fileName = global.estadual;
      } else {
